...
function agregarNumeros(numeros) {
  ...
  // Tu código:
  let sumaNumEnteros = 0;
  for(let i = 0; i<numeros.length; i++){
    sumaNumEnteros = sumaNumEnteros + numeros[i];
  }
return sumaNumEnteros;
}
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  let nroMasGrande = numeros[0];
  for(i=0; i<numeros.length; i++){
    if(numeros[i]>nroMasGrande){
...
function diaDeLaSemana(numeroDeDia) {
  ...
  //Escribe tu código aquí   
  for(let i = 0; numeroDeDia.length< i; i++){
...
return nuevoArray.push(nvoValor);
    }
  }
  return nuevoArray;
